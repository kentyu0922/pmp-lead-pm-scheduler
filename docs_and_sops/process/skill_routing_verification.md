...
## 7. 修复记录 (2026-09-04) — v4.1 optimized
- **P1 豁免死代码（R5）已修复**：`main.py` 在校准后调用 `fold_exempt_construction_permit`；苏州 200㎡/80万 用例移除「正式取得施工许可证」等政府节点，保留物业送审；下游场地移交前置重映射至图审/物业。
- **合规阻断**：compliance error 级触发 `COMPLIANCE_BLOCKED`，不再仅打日志后继续宣称 SUCCESS。
- **SKILL 契约**：允许 SS/FF+lag；撤销「100% FS only」与说明书冲突。
- **仍待买方 Windows+MSP**：四模板 × 免办/非免办 COM 实测。
